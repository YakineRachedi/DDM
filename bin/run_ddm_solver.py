...
from bin.config import Lx, Ly, nx, ny, k, ps
...
for J in J_values:
    ...
    final_residual = res_gmres[-1] if len(res_gmres) > 0 else None

    print(
        f"GMRES info = {info}, "
        f"iterations = {len(res_gmres)}, "
        f"final residual = {final_residual}"
    )
...
plt.ylabel(r"$\|r_k\|_2$")
plt.title(
    "Convergence GMRES selon le nombre de sous-domaines "
    "(taille de domaine fixe)"
)
plt.legend()
plt.grid(True, which="both", linestyle="--", alpha=0.5)

plt.savefig(BENCH_DIR / "GMRES_sub_domains.png")
plt.close()